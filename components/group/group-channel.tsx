...
export const GroupChannel = ({
  ...
  const { newMessage } = useChatSocket({ addKey: `chat:${channel.id}:messages`, updateKey: `chat:${channel.id}:messages:update`, queryKey: `chat:${channel.id}` });
  ...
  return (
    ...
    >
      {newMessage && <div className="absolute  right-10 h-2 w-2 rounded-full bg-zinc-500 cursor-none" />}
      ...
      {channel.name !== "general" && channel.name !== "product(s) info"  && role !== MemberRole.GUEST && (
